...
-- 允許所有已認證用戶讀取生效中的費率
DROP POLICY IF EXISTS "Allow authenticated users to read active rates" ON rate_configs;
CREATE POLICY "Allow authenticated users to read active rates"
ON rate_configs FOR SELECT
TO authenticated
USING (status = 'active');

-- 允許所有用戶讀取（暫時性政策，可根據需求調整）
DROP POLICY IF EXISTS "Allow all to read active rates" ON rate_configs;
CREATE POLICY "Allow all to read active rates"
ON rate_configs FOR SELECT
USING (status = 'active');

DROP POLICY IF EXISTS "Allow all to read tiers" ON membership_tiers;
